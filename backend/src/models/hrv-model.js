...
/**
 * Store HRV data in the database
 * @param {number} userId - User ID
 * @param {string} date - Date in format YYYY-MM-DD
 * @param {Object} hrvData - HRV data to store
...
const storeHrvData = async (userId, date, hrvData) => {
  try {
    ...
    if (entries.length === 0) {
      ...
      console.log(`Creating basic entry for date ${date} because HRV data requires it`);
      try {
    ...
    const readiness = hrvData.readiness !== undefined ? parseFloat(hrvData.readiness) : null;
    const stress = hrvData.stress_index !== undefined ?
                   parseFloat(hrvData.stress_index) :
                   (hrvData.stress !== undefined ? parseFloat(hrvData.stress) : null);
    const bpm = hrvData.mean_hr_bpm !== undefined ?
                parseInt(hrvData.mean_hr_bpm) :
                (hrvData.bpm !== undefined ? parseInt(hrvData.bpm) : null);
    const sdnnMs = hrvData.sdnn_ms !== undefined ?
                  parseFloat(hrvData.sdnn_ms) :
                  (hrvData.sdnnMs !== undefined ? parseFloat(hrvData.sdnnMs) : null);
    ...
    throw customError('Database error storing HRV data', 500);
  }
};
 ...
 * @param {number} userId - User ID
 * @param {string} date - Date in format YYYY-MM-DD
 * @returns {Object|null} HRV data or null if not found
 */
const getHrvData = async (userId, date) => {
  try {
    ...
    throw customError('Database error getting HRV data', 500);
  }
};
